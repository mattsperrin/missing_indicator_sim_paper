...
#### plotting
library(tidyverse)
sim <- read_csv("../output/sims_summary_SE_gathered.csv", guess_max = 100000)
...
ggp_i_ii_se <- sim_i_ii %>% 
  ggplot(aes(x = beta_U, y = value, group = Model, color = Model, shape =`SE type`)) +
  ...
  labs(y = "Standard Error", x = expression(beta[U])) +
...
ggp_ii_iii_eq_se <- sim_ii_iii_eq %>% 
  ggplot(aes(x = alpha_U, y = value, group = Model, color = Model, shape =`SE type`)) +
  facet_grid(fct_rev(sigma_Y) ~ parameter, scales = "free", labeller = label_parsed) +
  geom_point(position = position_dodge2(width = 0.5, reverse = TRUE), size = 2) +
...
#  geom_hline(aes(yintercept = loc, linetype = type), vline_aes) +
  theme_minimal(base_size = 22) +
  labs(y = "Standard Error", x = expression(alpha[U])) +
  scale_color_manual(values = cols) +
  guides(linetype = FALSE) +
  coord_flip()
...
ggp_iv_se <- sim_iv %>% 
  ggplot(aes(x = beta_A, y = value, group = Model, color = Model, shape =`SE type`)) +
  facet_grid(fct_rev(sigma_Y) ~ parameter, scales = "free", labeller = label_parsed) +
  geom_point(position = position_dodge2(width = 0.5, reverse = TRUE), size = 2) +
#  geom_linerange(aes(ymin = low, ymax = high), position = position_dodge2(width = 0.5, reverse = TRUE), size = 1.2) +
 # geom_hline(aes(yintercept = loc, linetype = type), vline_aes) +
  theme_minimal(base_size = 22) +
  labs(y = "Standard Error", x = expression(beta[A])) +
  scale_color_manual(values = cols) +
  guides(linetype = FALSE) +
  coord_flip()
